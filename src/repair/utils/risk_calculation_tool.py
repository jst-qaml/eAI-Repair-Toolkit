...
def run(**kwargs):
    ...
    if common_args.calc_target in [CALC_TARGETS[0], CALC_TARGETS[2]]:
        ...
        response["scene_prob"] = _scene_prob_output_by_json(
    ...
    if common_args.calc_target in [CALC_TARGETS[1], CALC_TARGETS[2]]:
        ...
        miss_rate_result = _calc_misrecognition_per_model(
            common_args, input_images, predict_args
        )
        response["miss_rate"] = _miss_rate_output_by_json(miss_rate_result)
        _generate_dataset_of_miss_rate(
            common_args.output_dir, image_id_to_path, miss_rate_result
        )

    with open(common_args.output_dir / "results.json", "w") as f:
        f.write(
            json.dumps(
                {"response": response}, indent=4 if common_args.format_json else None
            )
        )
...
def _get_input_images(h5_dataset_path):
    ...
    for h5_path in h5_paths:
        with h5py.File(h5_path, "r") as hf:
            ...
            attribute_ids = [
                attribute_ids.decode() for attribute_ids in hf["attribute_ids"][()]
            ]
...
def _calc_misrecognition_per_model(common_args, input_images, predict_args):
    ...
    for h5_path in h5_paths:
        ...
        images_generator = dataset.get_generator_for_risk_calculation_tool(
...
class EAIDatasetForRiskCalculationTool(eai_dataset.EAIDataset):
    ...
    def get_generator_for_risk_calculation_tool(self, progress_bar, target_indices):
        ...
        index_handler = eai_dataset.EAIIndexHandler(
            len(target_indices), self.batch_size, indices
        )
        return tf.data.Dataset.from_generator(
            EAIGeneratorForRiskCalculationTool(
                progress_bar, index_handler, self.hf["images"]
            ),
            ...
            for _ in range(batch_size):
                if len(batch) < batch_size:
                    batch = np.concatenate(
                        (batch, self.dataset[: batch_size - len(batch)])
                    )
                else:
                    break
            yield batch
