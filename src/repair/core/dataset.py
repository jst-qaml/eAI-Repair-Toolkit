...
class RepairDataset(RepairClass):
    ...
    def divide_train_dataset(
        ...
        sample_list = random.sample(range(len(train_images)), int(len(train_images) * divide_rate))
        # must be sorted by descendant order to prevent index out of bounds
