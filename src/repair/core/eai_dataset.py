...
class EAIDataset:
    ...
    def get_generators_split_validation(self, validation_size=0.2):
        ...
        validation_indices = train_indices[: int(math.floor(validation_size * len(train_indices)))]
        train_indices = train_indices[int(math.floor(validation_size * len(train_indices))) :]
        train_index_handler = EAIIndexHandler(self.image_shape[0], self.batch_size, train_indices)
    ...
    def get_test_data_generator(self):
        ...
        dataset = tf.data.Dataset.from_generator(
            ...
            output_signature=(
                ...
                tf.TensorSpec(shape=(self.batch_size, self.label_shape[1]), dtype=self.label_type),
            ),
        )
    ...
    def sample_from_file(self, sample_amount):
        ...
        if total != sample_amount:
            ...
            available_labels = [label for label, amount in label_pools.items() if amount > 0]
