...
class HydraModel(model.RepairModel):
    ...
    def _add_branch_layer(self, out, branch_list, output_shape):
        for idx in range(self.branch_num):
            ...
            b_out = Dense(
                output_shape, activation="softmax", name="Branch" + str(idx) + "_Dense"
            )(b_out)
        ...
        if "layers" in self.tuning:
            for tuning in self.tuning["layers"]:
                try:
                    ...
                    Exception("Wrong inputs for " + tuning[0])
        g_out = Dense(self.branch_num, activation="softmax", name="Gate" + "_Dense")(
            g_out
        )
    ...
    def add_block(self, out, d_num, w_val, channels, shortcut=True, name=None):
        ...
        sc = layers.BatchNormalization(
            axis=3, epsilon=1.001e-5, name=name + "_Shortcut2"
        )(sc)
        for n in range(d_num):
            ...
            out = layers.BatchNormalization(
                axis=3, epsilon=1.001e-5, name=b_name + "_BatchNorm1"
            )(out)
            out = SeparableConv2D(
                channels * w_val,
                (3, 3),
            ...
            out = layers.BatchNormalization(
                axis=3, epsilon=1.001e-5, name=b_name + "_BatchNorm2"
            )(out)
