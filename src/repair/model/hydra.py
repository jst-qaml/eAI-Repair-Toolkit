...
class HydraModel(model.RepairModel):
        ...
        for idx in range(self.branch_num):
            ...
            if "layers" in self.tuning:
                for tuning in self.tuning["layers"]:
                    ...
                    branch_tuning["name"] = "Branch" + str(idx) + branch_tuning["name"]
                    try:
            ...
            b_out = Dense(output_shape, activation="softmax", name="Branch" + str(idx) + "_Dense")(
                b_out
            )
        ...
        if "layers" in self.tuning:
            for tuning in self.tuning["layers"]:
                try:
                    ...
                    Exception("Wrong inputs for " + tuning[0])
        g_out = Dense(self.branch_num, activation="softmax", name="Gate" + "_Dense")(g_out)
    ...
    def _get_original(self, input_shape, output_shape):
        ...
        for idx in range(3):
            out = self.add_block(
                out, d_num, ws_val, 128 * 2**idx, name="Block" + str(128 * 2**idx)
            )
        branches = []
        for idx in range(self.branch_num):
            branch = self.add_block(out, d_num, wb_val, 1024, name="Branch" + str(idx))
            branch = AveragePooling2D(
                pool_size=(7, 7),
                data_format="channels_last",
                strides=1,
                name="Branch" + str(idx) + "_AvgPool",
            )(branch)
            branch = Flatten(name="Branch" + str(idx) + "_Flatten")(branch)
            branch = Dense(output_shape, activation="softmax", name="Branch" + str(idx) + "_Dense")(
                branch
            )
    ...
    def add_block(self, out, d_num, w_val, channels, shortcut=True, name=None):
        ...
        sc = layers.BatchNormalization(axis=3, epsilon=1.001e-5, name=name + "_Shortcut2")(sc)
        for n in range(d_num):
            ...
            out = SeparableConv2D(
                ...
                data_format="channels_last",
                strides=1,
                name=b_name + "_SepConv2D1",
            )(out)
            out = layers.BatchNormalization(axis=3, epsilon=1.001e-5, name=b_name + "_BatchNorm1")(
                out
            )
            out = SeparableConv2D(
                channels * w_val,
                ...
                data_format="channels_last",
                strides=1,
                name=b_name + "_SepConv2D2",
            )(out)
            out = layers.BatchNormalization(axis=3, epsilon=1.001e-5, name=b_name + "_BatchNorm2")(
                out
            )

        return Add(name=name + "_Add")([sc, out])
