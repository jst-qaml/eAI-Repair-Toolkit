...
class KaminariUtils:
    ...
    def _get_kaminari_inputs(self, model, inputs, batch_size=32):
        ...
        if isinstance(model, Sequential):
            ...
            errmsg = (
                f"Unknown model type: {type(model)!r}"
                "Use Functional or Sequential Keras models."
            )
        ...
        if len(head_model.outputs) > 1:
            kaminari_inputs = [
                np.array(kaminari_input) for kaminari_input in kaminari_inputs
            ]
        else:
    ...
    def _get_reduced_functional_model(self, model):
        ...
        for layer in tail_layers:
            ...
            new_layer_inputs = [
                i for i in new_inputs if names_dict[i.name] in inbound_layer_names
            ]
