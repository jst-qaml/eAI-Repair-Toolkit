...
class Arachne(method.RepairMethod):
    ...
    def localize(self, model, input_neg, output_dir: Path, verbose=1):
        ...
        pool = self._compute_forward_impact(
            reshaped_model, input_neg, candidates, self.num_grad
        )
    ...
    def _reshape_target_model(self, model, input_neg):
        ...
        if self.target_layer is None:
            ...
            while (
                type(model.get_layer(index=layer_index)) is not keras.layers.core.Dense
        ...
        if type(model.get_layer(index=layer_index)) is not keras.layers.core.Dense:
            raise IndexError(
                "Invalid layer_index: "
                + str(layer_index)
    ...
    def optimize(
        ...
        locations = self._get_initial_particle_positions(
            weights, model, self.num_particles
        )
        ...
        for t in range(self.num_iterations):
            ...
            for n in trange(
                ...
                " (it={}/{})".format(t + 1, self.num_iterations),
            ):
    ...
    def _update_particle(
        ...
        for n_new_x in range(len(new_x)):
            new_weights.append(
                [new_x[n_new_x], layer_index[n_new_x], nw_i[n_new_x], nw_j[n_new_x]]
            )
    ...
    def _initialize_personal_best_scores(
        self, locations, model, input_pos_sampled, input_neg
    ):
    ...
    def _get_initial_particle_positions(self, weights, model, num_particles):
        ...
        :return:
        """
        locations = [[0 for j in range(len(weights))] for i in range(num_particles)]
        for n_w in trange(len(weights), desc="Initializing particles"):
            ...
            samples = np.random.default_rng().normal(
                loc=mu, scale=std, size=num_particles
            )
