...
import repair
from repair.cli.loader import (
    gather_repair_classes,
    import_repair_dataset,
    import_repair_method,
    import_repair_model,
    load_utils,
)
...
def test_dataset_loader_mod():
    loaded_class = import_repair_dataset(DemoDataset.get_name())

    assert loaded_class is DemoDataset


def test_dataset_loader_pkg():
    loaded_class = import_repair_dataset(PkgDemoDataset.get_name())
...
def test_dataset_loader_not_found():
    with pytest.raises(AttributeError):
        import_repair_dataset("unknown_method")


def test_method_loader():
    loaded_class = import_repair_method(DemoMethod.get_name())
...
def test_method_loader_not_found():
    with pytest.raises(AttributeError):
        import_repair_method("unknown_method")


def test_model_loader():
    loaded_class = import_repair_model(DemoModel.get_name())
...
def test_model_loader_not_found():
    with pytest.raises(AttributeError):
        import_repair_method("unknown_model")
...
def test_raise_name_conflict_error(mocker):
    ...
    with pytest.raises(KeyError):
        gather_repair_classes(repair.methods, repair.core.method.RepairMethod)
