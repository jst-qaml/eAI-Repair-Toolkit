...
class EvaluateTest:
    ...
    def test_evaluate(
        ...
        mocked_load_weights = mocker.patch("repair.methods.arachne.arachne.Arachne.load_weights")
        mocked_load_weights.return_value = list(map(lambda row: row[:3], localized_weights))
        ...
        mocker.patch("repair.methods.arachne.arachne.load_model_from_tf", return_value=model)
