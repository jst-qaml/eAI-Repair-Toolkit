...
class KaminariTest:
    ...
    def input_data(self):
        ...
        arachne = Arachne()
        arachne.set_options(**common_settings)

        inputs_neg = arachne.load_input_neg(INPUT_NEG_DIR)
        inputs_pos = arachne.load_input_pos(INPUT_POS_DIR)
        ...
        if not in_ci:
            ...
            inputs_pos_indexes = rnd.choice(range(len(inputs_pos[0])), size=20, replace=False)
            inputs_pos = (
    ...
    @pytest.mark.skip("Find way compatible to tf2")
    ...
    def test_get_reduced_model(self, model):
        ...
        input_layers = len(
            [lay for lay in reduced_model.layers if isinstance(lay, tf.keras.layers.InputLayer)]
        )
    ...
    @pytest.mark.skip(reason="need to modify to meet arachne's input neg")
