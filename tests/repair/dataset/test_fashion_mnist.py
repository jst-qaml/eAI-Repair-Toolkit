...
def prepare_dataset(tmp_path_factory, module_mocker):
    ...
    module_mocker.patch(
        ...
        return_value=(
            (np.zeros((250, 28, 28), dtype=np.uint8), np.zeros((250,), dtype=np.uint8)),
            (np.zeros((50, 28, 28), dtype=np.uint8), np.zeros((50,), dtype=np.uint8)),
        ),
    )
...
def test_dataset_name():
    assert FashionMNIST.get_name() == "fashion-mnist"


def test_get_label_map():
    label_map = FashionMNIST.get_label_map()
