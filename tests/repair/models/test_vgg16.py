...
from repair.model.vgg16 import VGG16FineTuningModel


def test_model_loadable():
    loaded_model = load_repair_model("vgg16")

    assert loaded_model is VGG16FineTuningModel


def test_model_structure(imagenet_shape):
    input_shape, output_shape = imagenet_shape

    model = VGG16FineTuningModel().compile(input_shape=input_shape, output_shape=output_shape)
